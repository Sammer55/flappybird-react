...
const Header = () => {
  ...
  const githubIconStyle: CSSProperties = {
    width: 20,
    height: 20,
    filter: "invert(1)",
  };

  const lastScoreStyle: CSSProperties = {
    position: "absolute",
    left: "50%",
  ...
  const madeWithLoveStyle: CSSProperties = {
    marginRight: 16,
    color: "white",
  };
    ...
    <header style={headerStyle}>
      <div style={leftContainerStyle}>
        <img
          ...
          alt="GitHub icon"
        />
        <a href={gitHubProject} style={{ color: "white" }}>
          GitHub project
        </a>
      </div>
      <p style={lastScoreStyle}>
        Last score:{" "}
        <strong style={{ color: "#58d858" }}>
          {localStorage.getItem("score") || 0}
        </strong>
      </p>
      <p style={madeWithLoveStyle}>
        made with ❤️ by <a href={sammer55}>Sammer55</a>
      </p>
    </header>
  );
};
